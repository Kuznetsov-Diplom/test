...
if "pipeline" not in st.session_state:
    ...
    pipeline.add_step(MediaPipeFaceDetector(name="face_detection", min_detection_confidence=0.85))
    st.session_state.pipeline = pipeline

pipeline = st.session_state.pipeline

# Боковая панель — шаги и параметры
...
# Слайдеры параметров (пока только для face_detection)
...
with col2:
    ...
    if ctx.video_frame:
        ...
        # Метрики (как на твоём скриншоте)
...
col_btn3.button("📤 Экспорт вектора", use_container_width=True, disabled=True)  # позже

st.info("Пока работает только **Шаг 1: Детекция лица (MediaPipe)**. Добавим остальные шаги по мере развития библиотеки.")